    ...
    <>
      <header className="bg-[#ffffff] w-full border-b border-black/5 fixed z-100">
        <div className="max-w-7xl h-14 sm:h-20 mx-auto px-6 py-4 flex items-center md:gap-8">
          ...
          {/* Desktop Navigation */}
          <nav className="hidden md:flex ml-auto items-center gap-8 text-md">
            {menuItems.map((item) => (
              <NavLink
                key={item.path}
                to={item.path}
          ...
          <button
            onClick={toggleMenu}
            className="md:hidden ml-auto flex items-center justify-center p-2 rounded-md text-black/70 hover:text-[#b88a1e] hover:bg-black/5 transition-colors duration-300"
